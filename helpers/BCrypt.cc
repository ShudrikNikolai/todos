...
std::string BCrypt::hashPassword(const std::string& password)
{
    ...
    int rc = PKCS5_PBKDF2_HMAC_SHA1(password.c_str(), password.length(), salt, sizeof(salt), 100000, sizeof(key), key);
    if (rc != 1)
    {
...
bool BCrypt::verifyPassword(const std::string& password, const std::string& salt_key_b64)
{
    ...
    int rc =
        PKCS5_PBKDF2_HMAC_SHA1(password.c_str(), password.length(), salt, sizeof(salt), 100000, sizeof(key2), key2);
    if (rc != 1)
    {
        std::cerr << "Error: PKCS5_PBKDF2_HMAC_SHA1 returned " << rc << std::endl;
        return false;
